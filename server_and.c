...
int main(){
	...
	rv = getaddrinfo(HOST, PORT_AND, &hints, &serverinfo);
	if (rv) {
	...
	printf( "The Server AND is up and running using UDP on port 22262.\n");
	while(1){
		addr_len = sizeof(their_addr);
		int data[2], count2, i;
		recvfrom(socketfd, (char *)& count2, sizeof count2 , 0,(struct sockaddr *)&their_addr, &addr_len);
